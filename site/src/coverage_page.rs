...
fn observed_html(row: &RowOutcome) -> Html {
    match row.verdict {
        ...
        RowVerdict::Contradicted => {
            ...
            let mismatch = probe
                .and_then(|p| p.mismatch.clone())
                .unwrap_or_else(|| "disagreed".to_string());
...
fn probe_detail(probe: &ProbeOutcome) -> Html {
    ...
    let class = if probe.status == ProbeStatus::Disagreed {
        "ds-oe-cov-probe is-disagreed"
    } else {
        "ds-oe-cov-probe"
    };
...
fn probing_description(state: &RunState) -> Option<String> {
    ...
    let mut text = format!(
        "{} / {} probed — {} agreed, {} disagreed",
        progress.done, progress.total, progress.agreed, progress.disagreed
    );
...
fn results(report: &CoverageReport, filters: &Filters) -> Html {
    ...
    let visible: Vec<&RowOutcome> = report
        .rows
        .iter()
