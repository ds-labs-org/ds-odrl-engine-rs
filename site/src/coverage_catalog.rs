...
fn render_duties(duties: &[DutyExpect]) -> String {
    ...
    duties
        .iter()
        .map(|d| format!("{}/{} resolved={}", d.policy_id, d.action, d.resolved))
        .collect::<Vec<_>>()
        .join(", ")
}
...
pub fn classify_probe(
    ...
    if !is_known_decision(&response.decision) {
        return (
            ProbeStatus::Errored,
            Some(format!(
                "engine.wasm answered `{}`, which is not one of Allow/Deny/Error",
                response.decision
            )),
        );
    }
    if response.decision != expect.decision {
        return (
            ...
            Some(format!(
                "expected {}, observed {}",
                expect.decision, response.decision
            )),
        );
    }
    for needle in &expect.reason_contains {
        if !response.reason.contains(needle.as_str()) {
            return (
                ProbeStatus::Disagreed,
                Some(format!("reason missing `{needle}`")),
            );
        }
    }
    for needle in &expect.reason_excludes {
        if response.reason.contains(needle.as_str()) {
            return (
                ProbeStatus::Disagreed,
                Some(format!("reason contained excluded `{needle}`")),
            );
        }
    }
    ...
    if let Some(expected) = &expect.dataset_id {
        if &response.dataset_id != expected {
            return (
                ProbeStatus::Disagreed,
                Some(format!(
                    "dataset_id expected `{expected}`, observed `{}`",
                    response.dataset_id
                )),
            );
        }
...
pub fn parse_coverage_catalog(text: &str) -> Result<CoverageFile, String> {
    let file: CoverageFile = serde_json::from_str(text)
        .map_err(|err| format!("{COVERAGE_URL} did not match the expected shape: {err}"))?;

    if file.schema != COVERAGE_SCHEMA {
        return Err(format!(
            "{COVERAGE_URL} declares schema `{}`, this page speaks `{COVERAGE_SCHEMA}`",
            file.schema
        ));
    }
    ...
    for row in &file.rows {
        match (row.probe_ids.is_empty(), row.documented_because.is_some()) {
            (true, false) => {
                return Err(format!(
                    "{COVERAGE_URL}: row `{}` has neither probes nor a documented_because",
                    row.id
                ))
            }
            (false, true) => {
                return Err(format!(
                    "{COVERAGE_URL}: row `{}` has both probes and a documented_because",
                    row.id
                ))
            }
            _ => {}
        ...
        for probe_id in &row.probe_ids {
            if !file.probes.iter().any(|probe| &probe.id == probe_id) {
                return Err(format!(
                    "{COVERAGE_URL}: row `{}` references unknown probe `{probe_id}`",
                    row.id
                ));
            }
        }
    ...
    let rows: Vec<RowOutcome> = catalog
        ...
        .map(|row| {
            let probes: Vec<&ProbeOutcome> =
                row.probe_ids.iter().filter_map(|id| find(id)).collect();
            let verdict = derive_verdict(row, &probes);
            RowOutcome {
                row: row.clone(),
                verdict,
                probes: probes.into_iter().cloned().collect(),
            }
        })
        .collect();

    let count_status =
        |status: ProbeStatus| outcomes.iter().filter(|o| o.status == status).count() as u64;
    let count_verdict =
        |verdict: RowVerdict| rows.iter().filter(|r| r.verdict == verdict).count() as u64;
...
mod tests {
    ...
    const LATEST_COVERAGE_JSON: &str =
        include_str!("../../compliance/reports/latest-coverage.json");
    ...
    fn an_exactly_matching_response_agrees() {
        let (status, mismatch) =
            classify_probe(&expectation("Allow"), &response("Allow", "anything"));
    ...
    fn a_missing_reason_substring_disagrees_naming_the_substring() {
        ...
        assert_eq!(
            mismatch.as_deref(),
            Some("reason missing `(closed default)`")
        );
    }
    ...
    fn an_excluded_reason_substring_disagrees() {
        ...
        let (status, mismatch) = classify_probe(
            &expect,
            &response("Allow", "permission[0] (uid urn:rule:r-7)"),
        );
        assert_eq!(status, ProbeStatus::Disagreed);
    ...
    fn an_asserted_dataset_id_is_compared() {
        ...
        assert!(mismatch
            .unwrap()
            .contains("dataset_id expected `urn:asset:A`"));
    }
    ...
    fn an_out_of_contract_decision_is_errored_never_agreed_and_never_disagreed() {
        for decision in ["Permit", "allow", ""] {
            let (status, mismatch) =
                classify_probe(&expectation("Allow"), &response(decision, "r"));
        ...
        assert_eq!(
            classify_probe(&expectation("Maybe"), &response("Maybe", "r")).0,
            ProbeStatus::Errored
        );
    }
    ...
    fn a_malformed_or_truncated_response_is_errored_and_does_not_panic() {
        ...
        for body in [
            ...
            assert!(
                outcome.reason.unwrap().contains("Section 5.2's envelope"),
                "body {body:?}"
            );
        }
    }
    ...
    fn an_abi_level_failure_becomes_an_errored_probe_keeping_the_expected_decision() {
        let file =
            parse_coverage_catalog(LATEST_COVERAGE_JSON).expect("the committed catalog parses");
        let outcome =
            errored_probe_outcome(&file.probes[0], "evaluate() did not return a BigInt (i64)");

        assert_eq!(outcome.status, ProbeStatus::Errored);
        assert_eq!(outcome.expected_decision, file.probes[0].expect.decision);
        assert_eq!(
            outcome.reason.as_deref(),
            Some("evaluate() did not return a BigInt (i64)")
        );
    }
    ...
    fn derive_verdict_covers_all_four_verdicts_and_the_contradicted_precedence() {
        ...
        assert_eq!(
            derive_verdict(&row, &[&agreed, &agreed]),
            RowVerdict::Verified
        );
        assert_eq!(
            derive_verdict(&row, &[&agreed, &disagreed]),
            RowVerdict::Contradicted
        );
        assert_eq!(
            derive_verdict(&row, &[&agreed, &errored]),
        ...
        assert_eq!(
            derive_verdict(&row, &[&errored, &disagreed]),
            RowVerdict::Contradicted
        );
    ...
    fn a_catalog_with_the_wrong_schema_or_nothing_in_it_is_rejected() {
        ...
        assert!(
            parse_coverage_catalog(&base("something-else@9", row, probe))
                .unwrap_err()
                .contains("declares schema")
        );
        assert!(parse_coverage_catalog(&base(COVERAGE_SCHEMA, row, ""))
            .unwrap_err()
            .contains("no probes"));
        assert!(parse_coverage_catalog(&base(COVERAGE_SCHEMA, "", probe))
            .unwrap_err()
            .contains("no vocabulary rows"));

        let truncated = &LATEST_COVERAGE_JSON[..LATEST_COVERAGE_JSON.len() / 2];
        assert!(parse_coverage_catalog(truncated)
            .unwrap_err()
    ...
    fn a_row_referencing_an_unknown_probe_is_rejected_rather_than_silently_probing_nothing() {
      ...
      "probes":[{"id":"p","kind":"positive","title":"t","asserts":"a","falsified_by":"f","request":{},
                 ...
                 "dataset_id":null}}]}"#;
        assert!(parse_coverage_catalog(text)
            .unwrap_err()
            .contains("references unknown probe `nope`"));
    }
    ...
    fn a_row_that_is_both_probed_and_documented_or_neither_is_rejected() {
        ...
        assert!(parse_coverage_catalog(&with_rows(both))
            .unwrap_err()
            .contains("both probes and a documented_because"));
        assert!(parse_coverage_catalog(&with_rows(neither))
            .unwrap_err()
            .contains("neither probes nor"));
    }
    ...
    fn compile_coverage_report_tallies_both_axes_and_keeps_catalog_order() {
        ...
        let outcomes: Vec<ProbeOutcome> = catalog
            .probes
            .iter()
            .map(|p| probe_outcome(&p.id, ProbeStatus::Agreed))
            .collect();
        ...
        assert_eq!(
            report.verified + report.documented,
    ...
    fn one_disagreeing_probe_turns_exactly_the_rows_that_name_it_contradicted() {
        ...
        assert!(
            ...
            "the probe this test perturbs must be referenced by at least one row"
        );
        ...
        let contradicted: Vec<&str> = report
            .contradicted_rows()
            .iter()
            .map(|r| r.row.id.as_str())
            .collect();
    ...
    fn the_committed_catalog_parses_and_declares_this_pages_schema() {
        let file =
            parse_coverage_catalog(LATEST_COVERAGE_JSON).expect("the committed artifact parses");

        assert_eq!(file.schema, COVERAGE_SCHEMA);
        assert_eq!(
            file.rows.len(),
            52,
            "the source gap analysis enumerates 52 vocabulary rows"
        );
    ...
    fn every_committed_probe_is_referenced_by_at_least_one_row() {
        ...
        for probe in &file.probes {
            assert!(
                file.rows
                    .iter()
                    .any(|row| row.probe_ids.contains(&probe.id)),
    ...
    fn exactly_two_committed_rows_are_documented_only_and_each_says_why() {
        let file =
            parse_coverage_catalog(LATEST_COVERAGE_JSON).expect("the committed artifact parses");
        let documented: Vec<&CatalogRow> = file
            .rows
            .iter()
            .filter(|row| row.is_documented_only())
            .collect();
        ...
        for row in documented {
            assert!(
                row.documented_because
                    .as_ref()
                    .is_some_and(|why| !why.is_empty()),
                "row {} has an empty why",
                row.id
            );
        }
    }
    ...
    fn every_committed_probes_request_is_a_complete_section_5_2_envelope() {
        ...
        for probe in &file.probes {
            ...
            assert!(
                matches!(probe.expect.decision.as_str(), "Allow" | "Deny" | "Error"),
                "{}",
                probe.id
            );
            assert!(
                matches!(probe.kind.as_str(), "positive" | "negative"),
                "{}",
                probe.id
            );
        }
    ...
    fn every_committed_probe_can_both_agree_and_be_made_to_disagree() {
        ...
        for probe in &file.probes {
          ...
          probe.id
            ...
            let other = if expect.decision == "Allow" {
                "Deny"
            } else {
                "Allow"
            };
            let disagreeing = EngineResponse {
            ...
            assert_eq!(
                status,
                ProbeStatus::Disagreed,
                "{}: flipping the decision must disagree",
                probe.id
            );
            assert_eq!(
                mismatch.as_deref(),
    ...
    fn perturbing_one_probes_expectation_turns_its_rows_contradicted() {
        ...
        for probe_id in sample {
            assert!(
                file.probes.iter().any(|p| p.id == probe_id),
                "no probe {probe_id}"
            );
            ...
            assert!(
                !expected_rows.is_empty(),
                "{probe_id} is referenced by no row"
            );
    ...
    fn the_committed_catalog_carries_the_full_compliance_axis_on_both_of_its_halves() {
        ...
        let falls_short: Vec<&str> = file
            .probes
            .iter()
            .filter(|p| p.ideal.is_some())
            .map(|p| p.id.as_str())
            .collect();
        assert_eq!(
            falls_short.len(),
            15,
            "fifteen probes are judged short of full ODRL 2.2: {falls_short:?}"
        );

        for probe in file.probes.iter().filter(|p| p.ideal.is_some()) {
            let ideal = probe.ideal.as_ref().expect("just filtered");
            assert!(
                matches!(ideal.decision.as_str(), "Allow" | "Deny" | "Error"),
                "{}",
                probe.id
            );
        ...
        let with_gap: Vec<&str> = file
            .rows
            .iter()
            .filter(|r| r.full_compliance_gap.is_some())
            .map(|r| r.id.as_str())
            .collect();
    ...
    fn no_implemented_row_falls_short_and_every_falls_short_probe_belongs_to_an_implementable_row()
    {
        ...
        for row in file.rows.iter().filter(|r| r.status == "Implemented") {
            for probe_id in &row.probe_ids {
                assert!(
                    !ideal_of(probe_id),
                    "Implemented row {} names falls-short probe {probe_id}",
                    row.id
                );
            }
        }

        let implementable: Vec<&CatalogRow> =
            file.rows.iter().filter(|r| r.is_implementable()).collect();
        assert_eq!(
            implementable.len(),
            34,
            "the axis is scoped to 34 rows of the catalog's 52"
        );
    ...
    fn a_probes_raw_request_keeps_the_keys_this_sites_own_types_do_not_model() {
        ...
        for (probe_id, key) in [
            ...
            assert!(
                raw_of(probe_id).contains(key),
                "{probe_id}: the raw request no longer carries {key}"
            );
        }
    }
}
