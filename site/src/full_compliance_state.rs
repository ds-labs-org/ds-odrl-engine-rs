...
impl RunState {
    ...
    pub fn progress(&self) -> Option<&SpecProgress> {
        match self {
            RunState::Replaying { progress, .. } | RunState::Judging { progress, .. } => {
                Some(progress)
            }
            _ => None,
        }
...
mod tests {
    ...
    fn the_four_stages_are_ordered_and_the_last_one_names_the_spec_not_the_documentation() {
        ...
        assert_eq!(
            ...
            [
                ...
                "Replaying probes",
                "Judging against ODRL 2.2"
            ]
        );
        assert_eq!(
            Stage::ALL.iter().map(|s| s.order()).collect::<Vec<_>>(),
            [0, 1, 2, 3]
        );
    ...
    fn recording_outcomes_advances_done_and_exactly_one_tally_each_time() {
        ...
        assert_eq!(
            progress.meets + progress.falls_short + progress.undetermined,
            progress.done
        );
