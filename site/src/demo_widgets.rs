...
pub(crate) const DEFAULT_LEFT_OPERAND_SUGGESTIONS: &[&str] =
    &["sub", "nationality", "scope", "dateTime"];
...
pub fn ActionField(props: &ActionFieldProps) -> Html {
    ...
    let not_recognized = !props.recognized_actions.is_empty()
        && !trimmed.is_empty()
        && !props.recognized_actions.iter().any(|a| a == trimmed);
    let state = if not_recognized {
        InputState::Warning
    } else {
        InputState::Default
    };
