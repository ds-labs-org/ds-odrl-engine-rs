...
impl RunState {
    ...
    pub fn progress(&self) -> Option<&CoverageProgress> {
        match self {
            RunState::Probing { progress, .. } | RunState::Compiling { progress, .. } => {
                Some(progress)
            }
            _ => None,
        }
...
mod tests {
    ...
    fn each_state_reports_itself_as_the_current_stage_and_done_reports_none() {
        assert_eq!(
            RunState::LoadingWasm.current_stage(),
            Some(Stage::LoadingWasm)
        );
        assert_eq!(
            RunState::LoadingCatalog { engine_bytes: 1 }.current_stage(),
        ...
        for (state, complete) in sequence.iter().zip(expected_complete) {
            for stage in Stage::ALL {
                ...
                assert!(
                    ...
                    "a healthy run never marks a stage failed"
                );
            }
        }
    }
    ...
    fn a_failure_marks_only_the_earlier_stages_complete_and_leaves_the_later_ones_pending() {
        ...
        assert!(
            !state.is_complete(Stage::LoadingCatalog),
            "the failing stage is not complete"
        );
    ...
    fn a_finished_run_marks_every_stage_complete_and_none_current() {
        ...
        assert_eq!(
            state.progress(),
            None,
            "a finished run's payload is the report, not a counter"
        );
    }
    ...
    fn progress_is_exposed_only_while_probing_and_compiling() {
        ...
        assert_eq!(
            RunState::LoadingCatalog { engine_bytes: 1 }.progress(),
            None
        );
        assert_eq!(
            RunState::Probing {
    ...
    fn recording_probe_outcomes_advances_done_and_exactly_one_tally_each_time() {
        ...
        assert_eq!(
            progress.agreed + progress.disagreed + progress.errored,
            progress.done
        );
