...
use crate::compliance_run::{run, RunProgress, RunState, Stage};
use crate::pages::{case_study_credit, stat_row_html, STAT_ROW_CSS};
...
fn evaluating_description(state: &RunState) -> Option<String> {
    ...
    let mut text = format!(
        ...
        progress.done, progress.total, progress.passed, progress.failed
    );
...
fn results_table(
    ...
    let filtered: Vec<&CaseOutcome> = report
        ...
        .filter(|outcome| {
            query.is_empty()
                || outcome.slug.to_lowercase().contains(&query)
                || outcome.title.to_lowercase().contains(&query)
        })
        .collect();
