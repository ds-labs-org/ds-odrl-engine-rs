...
use crate::full_compliance::{
    contested_readings_for, shortfall_headline, unreached_narrowing, ContestedReading,
    FullComplianceReport, ProbeJudgment, RowJudgment, RowSpecVerdict, SpecOutcome,
    CONTESTED_READINGS,
};
    ...
    match row.verdict {
        ...
        RowSpecVerdict::FallsShort => {
            let probe = row
                .probes
                .iter()
                .find(|p| p.outcome == SpecOutcome::FallsShort);
        ...
        RowSpecVerdict::MeetsFullSpec => {
            ...
            let unreached = row
                .row
                .is_implementable()
                .then(|| unreached_narrowing(&row.row.id))
                .flatten();
            html!(
              <span class="ds-oe-fc-target">
        ...
        RowSpecVerdict::Undetermined => {
            let probe = row
                .probes
                .iter()
                .find(|p| p.outcome == SpecOutcome::Undetermined);
            html!(
              <span class="ds-oe-fc-target">
...
fn probe_detail(probe: &ProbeJudgment) -> Html {
    let class = if probe.outcome == SpecOutcome::FallsShort {
        "ds-oe-fc-probe is-short"
    } else {
        "ds-oe-fc-probe"
    };
    ...
    let contested = CONTESTED_READINGS
        .iter()
        .any(|entry| entry.probe == probe.id);
...
fn status_filter_onchange(
    filter: &UseStateHandle<StatusFilter>,
    value: StatusFilter,
) -> Callback<()> {
...
fn verdict_filter_onchange(
    filter: &UseStateHandle<VerdictFilter>,
    value: VerdictFilter,
) -> Callback<()> {
    let filter = filter.clone();
    Callback::from(move |()| filter.set(value))
...
fn matches_filters(row: &RowJudgment, filters: &Filters, query: &str) -> bool {
    ...
    row.row.term.to_lowercase().contains(query)
        ...
        || row
            .probes
            .iter()
            .any(|probe| probe.id.to_lowercase().contains(query))
}

fn results(report: &FullComplianceReport, filters: &Filters) -> Html {
    let query = filters.search.trim().to_lowercase();
    let visible: Vec<&RowJudgment> = report
        .rows
        .iter()
        .filter(|row| matches_filters(row, filters, &query))
        .collect();
