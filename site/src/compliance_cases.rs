...
pub fn evaluated_outcome(fixture: &CaseFixture, response_json: &str) -> CaseOutcome {
    let Some(expected) = fixture.expected_decision.clone() else {
        return errored_outcome(
            fixture,
            "artifact case carries a `request` but no `expected_decision`",
        );
    };
...
pub fn non_evaluated_outcome(fixture: &CaseFixture) -> CaseOutcome {
    match &fixture.skip_reason {
        ...
        None => errored_outcome(
            fixture,
            "artifact case carries neither a `request` nor a `skip_reason`",
        ),
    }
}
...
pub fn parse_case_file(text: &str) -> Result<CaseFile, String> {
    let file: CaseFile = serde_json::from_str(text)
        .map_err(|err| format!("{CASES_URL} did not match the expected shape: {err}"))?;

    if file.schema != CASES_SCHEMA {
        return Err(format!(
            "{CASES_URL} declares schema `{}`, this page speaks `{CASES_SCHEMA}`",
            file.schema
        ));
    }
...
pub fn parse_baseline(text: &str) -> Result<BaselineReport, String> {
    serde_json::from_str(text)
        .map_err(|err| format!("{BASELINE_URL} did not match the expected shape: {err}"))
}
...
pub fn compile_report(
    ...
    let (passed, failed, skipped, errored) = (
        count(CaseStatus::Passed),
        count(CaseStatus::Failed),
        count(CaseStatus::Skipped),
        count(CaseStatus::Errored),
    );
...
mod tests {
    ...
    fn fixture(expected: Option<&str>, skip_reason: Option<&str>) -> CaseFixture {
        CaseFixture {
            ...
            request: expected
                .map(|_| RawValue::from_string("{\"action\":\"read\"}".to_string()).unwrap()),
    ...
    fn evaluated_outcome_reads_a_real_engine_response_envelope() {
        ...
        assert_eq!(
            ...
            Some("permission[0] of policy 'p' matched: action 'read'")
        );
    }
    ...
    fn a_disagreement_with_the_suites_expected_decision_is_a_failure_carrying_both_sides() {
        let response =
            r#"{"dataset_id":"d","decision":"Deny","reason":"no permission matched","duties":[]}"#;
        let result = evaluated_outcome(&fixture(Some("Allow"), None), response);
    ...
    fn a_malformed_or_truncated_response_is_errored_and_does_not_panic() {
        for body in [
            ...
            assert!(
                result.reason.unwrap().contains("Section 5.2's envelope"),
                "body {body:?}"
            );
        }
    }
    ...
    fn an_out_of_contract_decision_string_is_errored_with_the_raw_value_kept() {
        let result = evaluated_outcome(
            &fixture(Some("Allow"), None),
            r#"{"decision":"Permit","reason":"x"}"#,
        );
    ...
    fn a_skipped_case_carries_the_artifacts_cited_reason_through_unchanged() {
        let result = non_evaluated_outcome(&fixture(
            None,
            Some("odrl:xone is not expressible (translate.rs::xone_unsupported)"),
        ));
    ...
    fn an_abi_level_failure_becomes_an_errored_case_keeping_the_expected_decision() {
        let result = errored_outcome(
            &fixture(Some("Deny"), None),
            "evaluate() did not return a BigInt (i64)",
        );
        assert_eq!(result.status, CaseStatus::Errored);
    ...
    fn an_agreeing_baseline_reports_no_divergence() {
        let baseline = baseline_of(vec![
            baseline_case("a", "passed"),
            baseline_case("b", "skipped"),
        ]);
        let outcomes = vec![
            outcome("a", CaseStatus::Passed),
            outcome("b", CaseStatus::Skipped),
        ];
    ...
    fn a_case_present_in_only_one_of_the_two_artifacts_is_a_divergence_in_both_directions() {
        let baseline = baseline_of(vec![
            baseline_case("a", "passed"),
            baseline_case("only-native", "passed"),
        ]);
        let outcomes = vec![
            outcome("a", CaseStatus::Passed),
            outcome("only-live", CaseStatus::Passed),
        ];
        let comparison = compare_to_baseline(&outcomes, &baseline);
        ...
        assert_eq!(
            ...
            vec![
                Divergence {
                    slug: "only-live".to_string(),
                    native: "absent".to_string(),
                    live: "passed".to_string()
                },
                Divergence {
                    slug: "only-native".to_string(),
                    native: "passed".to_string(),
                    live: "absent".to_string()
                },
            ]
    ...
    fn the_committed_corpus_parses_and_declares_this_pages_schema() {
        ...
        assert_eq!(
            file.cases.len(),
            68,
            "the vendored corpus currently indexes 68 cases"
        );
        ...
        for case in &file.cases {
            ...
            match (&case.request, &case.expected_decision, &case.skip_reason) {
                ...
                (None, None, Some(reason)) => {
                    assert!(!reason.is_empty(), "{}: empty skip_reason", case.slug)
                }
                other => panic!(
                    "{}: a case must be exactly evaluable or skipped, got {other:?}",
                    case.slug
                ),
            }
        }
    ...
    fn a_corpus_with_the_wrong_schema_or_no_cases_is_rejected_rather_than_half_parsed() {
        let wrong_schema =
            r#"{"schema":"something-else@9","suite":"s","cases":[{"slug":"a","title":"t"}]}"#;
        assert!(parse_case_file(wrong_schema)
            .unwrap_err()
        ...
        assert!(parse_case_file(truncated)
            .unwrap_err()
