...
/// Placeholder Demonstrator page: a later stage wires this to a real
/// `alloc`/`evaluate`/`dealloc` round trip against `engine.wasm` (see
/// engine/src/abi.rs) with an editable request and a rendered decision.
#[component]
  ...
  html!(
    <>
      <Content>
        ...
        <p>{ "This page will drive engine.wasm's evaluate() ABI export directly against an editable policy/claims request. Not wired up yet." }</p>
      </Content>
      { case_study_credit() }
    </>
  )
