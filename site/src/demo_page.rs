...
use crate::demo_widgets::{
    ActionField, ClaimRowsEditor, RuleRowsEditor, DEFAULT_LEFT_OPERAND_SUGGESTIONS,
    LEFT_OPERAND_DATALIST_ID,
};
...
fn field_onchange(
    form: UseStateHandle<DemoForm>,
    set: fn(&mut DemoForm, String),
) -> Callback<String> {
...
fn rules_onchange(
    form: UseStateHandle<DemoForm>,
    set: fn(&mut DemoForm, Vec<RuleRow>),
) -> Callback<Vec<RuleRow>> {
...
pub fn DemoPage() -> Html {
    ...
    let recognized_actions_from_profile: Vec<String> = loaded_profile
        .as_ref()
        .map(|p| p.actions.iter().map(|a| a.id.clone()).collect())
        .unwrap_or_default();
    let left_operand_suggestions: Vec<String> = {
        let mut suggestions: Vec<String> = DEFAULT_LEFT_OPERAND_SUGGESTIONS
            .iter()
            .map(|s| s.to_string())
            .collect();
