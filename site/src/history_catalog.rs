...
impl Release {
    ...
    pub fn meets_full_spec_probe_fraction(&self) -> Option<f64> {
        let full_compliance = self.full_compliance.as_ref()?;
        let total = full_compliance.probes_meets
            + full_compliance.probes_falls_short
            + full_compliance.probes_undetermined;
        if total == 0 {
            return None;
        }
...
impl HistoryFile {
    ...
    pub fn unaddressable(&self) -> Vec<&Release> {
        self.releases
            .iter()
            .filter(|release| release.coverage.is_none())
            .collect()
    }
...
pub fn parse_release_history(text: &str) -> Result<HistoryFile, String> {
    let file: HistoryFile = serde_json::from_str(text)
        .map_err(|err| format!("{HISTORY_URL} did not match the expected shape: {err}"))?;

    if file.schema != HISTORY_SCHEMA {
        return Err(format!(
            "{HISTORY_URL} declares schema `{}`, this page speaks `{HISTORY_SCHEMA}`",
            file.schema
        ));
    }
    ...
    for release in &file.releases {
        ...
        match (release.coverage.is_some(), release.coverage_error.is_some()) {
            (true, true) => {
                return Err(format!(
                    "{HISTORY_URL}: release `{}` carries both a coverage tally and an error",
                    release.tag
                ))
            }
            (false, false) => {
...
mod tests {
    ...
    fn the_committed_artifact_parses_and_covers_the_whole_tag_range() {
        ...
        assert!(
            file.releases.len() >= 25,
            "expected every tag, found {}",
            file.releases.len()
        );
        assert_eq!(
            file.releases.first().map(|r| r.tag.as_str()),
            Some("v0.1.0")
        );
    ...
    fn the_newest_release_agrees_with_the_current_catalog() {
        ...
        let coverage = latest
            .coverage
            .as_ref()
            .expect("the newest release is addressable by its own catalog");
        assert_eq!(coverage.probes_total, file.catalog.probes);
        assert_eq!(
            coverage.verified + coverage.contradicted + coverage.inconclusive + coverage.documented,
            file.catalog.rows
        );
        assert_eq!(
            coverage.contradicted, 0,
            "the newest tag must not contradict the catalog generated from it"
        );
    }
    ...
    fn structural_gap_count_is_identical_across_every_addressable_release() {
        let file = committed();
        let addressable: Vec<&FullComplianceTally> = file
            .releases
            .iter()
            .filter_map(|r| r.full_compliance.as_ref())
            .collect();
        assert!(
            addressable.len() >= 2,
            "need at least two addressable releases to compare"
        );
        let first_structural_gap = addressable[0].rows_structural_gap;
        for (release, full_compliance) in file
            .releases
            .iter()
            .filter(|r| r.full_compliance.is_some())
            .zip(addressable.iter())
        {
            assert_eq!(
                full_compliance.rows_structural_gap, first_structural_gap,
                "{}: structural gap count moved",
                release.tag
            );
        }
    }
    ...
    fn an_unaddressable_release_reports_no_fraction_rather_than_zero() {
        ...
        assert!(
            ...
            "this history contains a real pre-v0.6.0 wire break"
        );
    ...
    fn meets_full_spec_probe_fraction_is_a_finer_grain_than_the_row_fraction() {
        let file = committed();
        let addressable: Vec<&Release> = file
            .releases
            .iter()
        ...
        for release in addressable {
            ...
            let probe_fraction = release
                .meets_full_spec_probe_fraction()
                .unwrap_or_else(|| panic!("{}: expected Some", release.tag));
            let probe_total = full_compliance.probes_meets
                + full_compliance.probes_falls_short
                + full_compliance.probes_undetermined;
            ...
            assert_eq!(
                probe_fraction,
                full_compliance.probes_meets as f64 / probe_total as f64,
                "{}",
                release.tag
            );
            assert!(
                (0.0..=1.0).contains(&probe_fraction),
                "{}: {probe_fraction} out of range",
                release.tag
            );
    ...
    fn a_tally_that_does_not_add_up_is_rejected() {
        ...
        let err =
            parse_release_history(&file.to_string()).expect_err("a broken tally must be rejected");
