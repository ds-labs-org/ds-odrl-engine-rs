...
use crate::coverage_catalog::{
    CatalogRow, Category, CoverageFile, Ideal, ProbeFixture, ProbeOutcome, ProbeStatus,
};
...
impl RowSpecVerdict {
    ...
    pub fn is_short_of_spec(self) -> bool {
        matches!(
            self,
            RowSpecVerdict::FallsShort | RowSpecVerdict::StructuralGap
        )
    }
}
...
impl FullComplianceReport {
    ...
    pub fn short_rows(&self) -> Vec<&RowJudgment> {
        self.rows
            .iter()
            .filter(|row| row.verdict.is_short_of_spec())
            .collect()
    }
}
...
pub fn derive_row_verdict(row: &CatalogRow, probes: &[&ProbeJudgment]) -> RowSpecVerdict {
    ...
    if probes
        .iter()
        .any(|p| p.outcome == SpecOutcome::Undetermined)
    {
        return RowSpecVerdict::Undetermined;
    }
...
pub fn compile_full_compliance_report(
    ...
    let count_verdict =
        |verdict: RowSpecVerdict| rows.iter().filter(|r| r.verdict == verdict).count() as u64;
    let count_status = |status: &str| rows.iter().filter(|r| r.row.status == status).count() as u64;
    let count_outcome =
        |outcome: SpecOutcome| judged.iter().filter(|p| p.outcome == outcome).count() as u64;
...
mod tests {
    ...
    fn the_seven_out_of_scope_rows_are_excluded_and_nothing_else_is() {
        ...
        assert_eq!(
            report.rows_excluded, 7,
            "the profile-extension and odrl:hasPolicy rows"
        );
        assert_eq!(report.rows_in_scope, 45);
        assert_eq!(
            report.rows_in_scope + report.rows_excluded,
            catalog.rows.len() as u64
        );
        for row in &report.rows {
            assert_ne!(
                row.row.status, "OutOfScope",
                "row {} should have been excluded",
                row.row.id
            );
        }
        assert_eq!(
        ...
        assert_eq!(
            (
                report.implemented_rows,
                report.partial_rows,
                report.not_implemented_rows
            ),
            (11, 23, 11)
        );
    }
    ...
    fn the_row_axis_tallies_to_the_numbers_this_page_states() {
        ...
        assert_eq!(
            report.rows_falls_short, 13,
            "thirteen rows have a probe demonstrating a shortfall"
        );
        assert_eq!(
            report.rows_structural_gap, 1,
            "party.collections, which no request can pose"
        );
    ...
    fn the_probe_axis_counts_distinct_probes_and_only_a_minority_distinguish_the_two_questions() {
        ...
        assert_eq!(
            report.probes_judged, 130,
            "six probes are named only by rows this page excludes"
        );
    ...
    fn a_probe_whose_ideal_decision_equals_its_current_one_still_falls_short() {
        ...
        assert_eq!(
            ideal.decision, fixture.expect.decision,
            "the premise of this test: the decisions are equal"
        );
    ...
    fn no_shortfall_headline_claims_a_delta_between_two_equal_decisions() {
        ...
        for row in report.short_rows() {
            for probe in row
                .probes
                .iter()
                .filter(|p| p.outcome == SpecOutcome::FallsShort)
            {
                let ideal = probe
                    .ideal
                    .as_ref()
                    .expect("a falls-short probe carries the ideal that made it one");
            ...
            probe.id
                    ...
                    assert!(
                        headline.contains(&delta),
                        "probe {}: the delta is the evidence here: {headline}",
                        probe.id
                    );
                }
            }
    ...
    fn a_shortfall_headline_shows_a_delta_only_where_there_is_one() {
        ...
        assert!(
            !unchanged.contains("full ODRL 2.2 requires Allow"),
            "{unchanged}"
        );
        assert!(
            unchanged.contains("what must change is the duties list"),
            "{unchanged}"
        );

        let changed = shortfall_headline("p-differs", Some("Deny"), &ideal);
        assert!(
            changed.contains("this engine answers Deny, full ODRL 2.2 requires Allow"),
            "{changed}"
        );
        ...
        assert!(
            unobserved.contains("full ODRL 2.2 requires Allow"),
            "{unobserved}"
        );
    }
    ...
    fn every_implemented_row_meets_full_spec_and_carries_no_ideal_anywhere() {
        ...
        for row in report.rows.iter().filter(|r| r.row.status == "Implemented") {
            ...
            for probe in &row.probes {
                assert!(
                    probe.ideal.is_none(),
                    "Implemented row {} names falls-short probe {}",
                    row.row.id,
                    probe.id
                );
            }
    ...
    fn twenty_implementable_rows_meet_full_spec_on_the_probes_they_carry() {
        let report = report();
        let implementable: Vec<&RowJudgment> = report
            .rows
            .iter()
            .filter(|r| r.row.is_implementable())
            .collect();
    ...
    fn exactly_one_row_is_a_structural_gap_and_it_is_the_one_with_no_probes() {
        let report = report();
        let gaps: Vec<&RowJudgment> = report
            .rows
            .iter()
            .filter(|r| r.verdict == RowSpecVerdict::StructuralGap)
            .collect();

        assert_eq!(gaps.len(), 1);
        assert_eq!(gaps[0].row.id, "party.collections");
        assert!(
            gaps[0].probes.is_empty(),
            "a structural gap has no probe to expand"
        );
        assert!(
            gaps[0]
                .row
                .full_compliance_gap
                .as_ref()
                .is_some_and(|gap| gap.contains("party_collections")),
    ...
    fn the_ispartof_row_falls_short_with_berlin_and_germany_named_in_its_citation() {
        ...
        let row = report
            .rows
            .iter()
            .find(|r| r.row.id == "operators.is-part-of")
        ...
        let probe = row
            .probes
            .iter()
            .find(|p| p.id == "op-ispartof-no-hierarchy")
        ...
        let ideal = probe
            .ideal
            .as_ref()
            .expect("it falls short, so it carries an ideal");
        assert_eq!(
            ideal.decision, "Allow",
            "the engine denies where a compliant one allows"
        );
    ...
    fn an_errored_or_contradicting_probe_is_undetermined_and_never_either_verdict() {
        ...
        assert!(contradicting
            .undetermined_because
            .unwrap()
            .contains("departed from its own documented behaviour"));
        ...
        assert_eq!(
            ...
            13,
            "the isPartOf row left the shortfall bucket"
        );
    ...
    fn falls_short_out_ranks_undetermined_within_one_row() {
        ...
        let row = report
            .rows
            .iter()
            .find(|r| r.row.id == "operators.isa-haspart")
            .expect("the row exists");
        assert_eq!(row.verdict, RowSpecVerdict::FallsShort);
    }
    ...
    fn every_contested_reading_names_a_real_in_scope_row_and_a_probe_that_row_names() {
        ...
        for entry in CONTESTED_READINGS {
            ...
            assert!(
                is_in_scope(row),
                "contested reading names excluded row {}",
                entry.row
            );
            assert!(
            ...
            assert!(
                !entry.shipped.is_empty() && !entry.alternative.is_empty(),
                "{}",
                entry.probe
            );
        }
        ...
        assert_eq!(
            probes.len(),
            14,
            "pc-kind-nonsense is listed under both in-scope rows that name it"
        );
        assert_eq!(
            contested_readings_for("policy-classes.discrimination").len(),
            4
        );
    }
    ...
    fn the_unreached_narrowing_notes_cover_exactly_the_implementable_rows_that_meet_full_spec() {
        ...
        let mut meeting: Vec<&str> = report
            ...
            .iter()
            .filter(|row| {
                row.row.is_implementable() && row.verdict == RowSpecVerdict::MeetsFullSpec
            })
        ...
        assert_eq!(
            noted, meeting,
            "the notes and the live verdicts must describe the same set of rows"
        );
        ...
        for label in labels {
            for forbidden in ["greed", "erified", "ontradicted"] {
                assert!(
                    ...
                    "label {label:?} reuses /coverage's vocabulary"
                );
            }
        }
    }
